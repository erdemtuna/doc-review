...
test("a review keeps the CLI invocation resolved when its server starts", async () => {
  ...
  const originalStateDir = process.env.DOC_REVIEW_STATE_DIR;
  ...
  if (process.platform === "win32") {
    fs.writeFileSync(path.join(bin, "doc-review.cmd"), "@exit /b 0\r\n");
  } else {
    const command = path.join(bin, "doc-review");
  ...
  try {
    ...
    if (originalStateDir === undefined) delete process.env.DOC_REVIEW_STATE_DIR;
    else process.env.DOC_REVIEW_STATE_DIR = originalStateDir;
