...
| Comments and composer | Open Comments; edit a saved comment, try Delete then Cancel. Select new text; open a comment; scroll the nested panel. | Direct icon actions have labels/tooltips. Delete still requires confirmation. Draft, caret and selection survive surface changes. The textarea focus outline does not touch its keyboard hints. Back to selection reveals the original target. |
...
| Changes controls | HTML has two completed rounds. Switch Round, Page, Content/Source and Previous/Next/Jump to change. Open Comparison details. | Labels and content refer to the same selected round/page. Navigation, status/counts and diagnostic coverage agree. Switching view or theme does not replace the document. |
...
| Partial and empty states | HTML Round 1 / notes.md intentionally has Source-only coverage. The third example has no comments or rounds. | Missing Content is explained honestly; Source remains usable. Empty Comments and Changes are helpful, and Send is disabled when nothing is queued. |
| Responsive and keyboard | Repeat at 1440, 768, 390 and 320 pixels in both themes. Use Tab, Shift+Tab, Enter and Escape. | No clipped controls or unintended horizontal page overflow. Primary Send remains reachable while long inventory/secondary handoff scrolls. Focus is visible. Nonmodal review surfaces do not trap the authored document; confirmation dialogs do. |
